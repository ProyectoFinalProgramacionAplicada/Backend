...
namespace TruekAppAPI.Services
{
    public class WalletService : IWalletService
    {
        ...
        // ======================================================
        //  🔹 Transferencia asociada a un TRUEQUE
        //  fromUserId = quien PAGA (sale saldo)
        //  toUserId   = quien COBRA (entra saldo)
        //  RefType = "Trade", RefId = tradeId
        // ======================================================
        public async Task ApplyTradeTransferAsync(int fromUserId, int toUserId, decimal amount, int tradeId)
        {
            if (amount <= 0m)
                throw new ArgumentException("Amount must be positive", nameof(amount));
            ...
            if (fromUser.TrueCoinBalance < amount)
                throw new InvalidOperationException("Insufficient balance to complete trade");

            // Debitar al que paga (por el trueque)
            fromUser.TrueCoinBalance -= amount;
            _db.WalletEntries.Add(new WalletEntry
            {
                UserId = fromUserId,
                Amount = amount * -1m,
                Type = WalletEntryType.InternalTransferOut, // TradeSent
                RefType = "Trade",
                RefId = tradeId
            });

            // Acreditar al que cobra (por el trueque)
            toUser.TrueCoinBalance += amount;
            _db.WalletEntries.Add(new WalletEntry
            {
                UserId = toUserId,
                Amount = amount,
                Type = WalletEntryType.InternalTransferIn, // TradeReceived
                RefType = "Trade",
                RefId = tradeId
            });
